...
Items 1–71 are **implemented** (§ZZZZZZZZZZZZ added 2026-09-05: Boltz-2 binding-pose cache for structure-guided §MM growth vectors — extracts per-atom min-distance-to-protein from rank-0 PDB before file removal, stores in `BoltzWrapper._pose_exposure`, passed as `atom_exposure` to `run_salsa_search()` so fg_add/ring_walk probe solvent-exposed atoms first; ~150 lines across wrapper.py + salsa.py + miner.py). §AAAAAAAAAAAA (item 71) added 2026-09-04: GA population diversity injection from cross-epoch SQLite elite. Item 5 remains as a conditional direction (binding-pocket docking filter, only relevant when `binding_pocket` is set in config). Items 72–74 proposed 2026-09-06 (see BOLTZ2_INTEGRATION.md): §BBBBBBBBBBBB (intra-chunk PSICHIC batch surrogate pre-filter, ~40 lines), §CCCCCCCCCCCC (residue-type contact map for pharmacophore-guided fg_add, ~95 lines), §DDDDDDDDDDDD (fast-mode linear score calibration for §MM acceptance threshold, ~45 lines).
