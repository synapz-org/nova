...
All items 1–43 are **implemented**. Items 5 and 7 remain as conditional/research directions.
...
| 43 | §FFFFFFFFFFFFFF: Pre-§XX boltz_cache merge — move boltz_cache → all_scores merge from before §CC to before §XX so §XX/§TTTTTT/§WW/§UUUU see complete epoch picture (including §FF scores) when §MM ran 0 rounds | ~20 lines | ✅ Done | +2–5% better tautomer seed quality + correct top-2 candidates for §WW/§UUUU on epochs where §MM budget-expires immediately (~10–20% of A100 epochs) |
