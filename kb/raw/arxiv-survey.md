...
| 65 | §UUUUUUUUUUUU: Surrogate pre-filter for SALSA perturbations — before PSICHIC scoring of 200 perturbations, run dual RF surrogate (~20 ms) and keep only top-25% (cap 50); reduces PSICHIC calls per §MM round by ~75%, enabling 3–4× more rounds per epoch | ~40 lines | 📋 Proposed | +3–6% Boltz LE on epoch 2+ by enabling deeper §MM hill-climbing within same time budget |
