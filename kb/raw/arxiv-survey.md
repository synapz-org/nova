...
Items 1–74 are **implemented** (§BBBBBBBBBBBB/§CCCCCCCCCCCC/§DDDDDDDDDDDD added 2026-09-07: intra-chunk PSICHIC surrogate pre-filter, pharmacophore contact-map for fg_add, and fast-mode OLS calibration for §MM acceptance gate). §ZZZZZZZZZZZZ added 2026-09-05: Boltz-2 binding-pose cache for structure-guided §MM growth vectors — now returns `(dist_Å, contact_type)` tuples (§CCCCCCCCCCCC extends the format). §AAAAAAAAAAAA (item 71) added 2026-09-04: GA population diversity injection from cross-epoch SQLite elite. Item 5 remains as a conditional direction (binding-pocket docking filter, only relevant when `binding_pocket` is set in config). Items 75–76 **proposed** 2026-09-08.
...
| 63 | §SSSSSSSSSSSS: Cross-epoch SALSA operator win persistence — decay wins ÷2 at epoch boundary (vs full reset); save to SQLite on each credit; restore from SQLite on startup (cap 40 virtual trials); export/import via GitHub cache alongside rxn class weights | ~40 lines | ✅ Done (2026-08-25) | +1–3% Boltz LE on epoch 2+ warm-cache runs by starting §MM with informed operator priors; especially impactful on restarts where epoch 1 cold-starts previously had no §OOOO signal |
| 64 | §TTTTTTTTTTTT: Reaction-class dead-end de-emphasis — apply 0.5× weight to bottom-quartile (≥4 classes) rxn classes that have ≥10 Boltz-scored molecules and persistently low mean LE; implemented inside `_load_rxn_class_weights` alongside §EEEEEE top-K amplification | ~8 lines | ✅ Done (2026-08-27) | +1–3% Boltz LE on epoch 3+ by reallocating streaming budget away from consistently poor reaction templates |
| 65 | §UUUUUUUUUUUU: Surrogate pre-filter for SALSA perturbations — before PSICHIC scoring of 200 perturbations, run dual RF surrogate (~20 ms) and keep only top-25% (cap 50); reduces PSICHIC calls per §MM round by ~75%, enabling 3–4× more rounds per epoch | ~55 lines | ✅ Done (2026-08-28) | +3–6% Boltz LE on epoch 2+ by enabling deeper §MM hill-climbing within same time budget |
| 66 | §VVVVVVVVVVVV: Reliability-adjusted §MM seed selection — select §MM starting molecule by `boltz_le / (1 + 10 × max(boltz_le_std, boltz_ww_std))` rather than raw argmax; prevents hill-climbing from stochastic Boltz-2 outliers; consistent with surrogate training variance weights (§WWWWWWWWWWWWWWW/§XXXXXXXX) | ~50 lines | ✅ Done (2026-08-29) | +2–5% Boltz LE on ~20–35% of epochs where initial Boltz pass produces a high-variance outlier |
| 67 | §WWWWWWWWWWWW: Cache-adaptive surrogate blend alpha — scale `augment_pool_with_surrogate_blend` alpha from 0.60 (at RF threshold, 100 pts) to 0.80 (at 600+ pts) so a mature RF surrogate has proportionally more influence over SALSA/GA candidate direction as the cache grows; 6 call sites updated; `_rank_rxn_classes_by_surrogate` gets `alpha` parameter | ~35 lines | ✅ Done (2026-08-30) | +1–3% Boltz LE on warm-cache epochs (300+ pts) by better exploiting a well-trained target-specific surrogate |
| 68 | §XXXXXXXXXXXX: Cache-adaptive UCB beta — scale UCB exploration coefficient from 1.0 (at RF threshold) down to 0.5 (at 725+ pts) so a mature surrogate exploits mean predictions more and explores uncertain regions less; dual to §WWWWWWWWWWWW; 3 UCB call sites updated (§RRRR/§AAAAAA/§HHHHHHHHHH ranking) | ~30 lines | ✅ Done (2026-09-01) | +1–2% Boltz LE on warm-cache epochs (500+ pts); compounds with §WWWWWWWWWWWW for fully consistent exploit-as-you-learn annealing |
| 69 | §YYYYYYYYYYYY: Multi-start §MM SALSA from diversity-maximised cache seeds — after §VVVVVVVVVVVV selects reliability-adjusted primary seed, add up to 2 more seeds via max-min Tanimoto on `_mm_all_scored`; run §MM rounds for each seed sequentially within existing time budget; Tanimoto ≤ 0.15 guard prevents near-duplicate seeds; `_mm_tried_seeds` resets per seed; `_mm_best_score` global across seeds | ~75 lines | ✅ Done (2026-09-03) | +3–8% Boltz LE on epoch 3+ with ≥3 diverse cached scaffolds; escapes local optima that single-seed §MM cannot |
| 70 | §ZZZZZZZZZZZZ: Boltz-2 binding-pose cache for structure-guided §MM growth vectors — after each full Boltz call, extract per-atom min-distance-to-protein from output PDB before file removal; store in `BoltzWrapper._pose_exposure`; expose as `atom_exposure` mask to `run_salsa_search()` to up-weight `fg_add`/`ring_walk` at solvent-exposed atoms and down-weight at buried positions | ~150 lines | ✅ Done (2026-09-05) | +3–6% Boltz LE on epoch 2+ by redirecting growth operators away from sterically blocked positions; ~20–30% fewer fast-screen failures for fg_add/ring_walk |
| 71 | §AAAAAAAAAAAA: GA population diversity injection from cross-epoch SQLite elite — before `run_gradient_ga()`, query top-10 reliability-adjusted Boltz cache molecules, select top-3 by max-min Tanimoto diversity (reuses `_select_diverse_mm_seeds()`), inject into GA initial population replacing worst-PSICHIC members; injected molecules participate in crossover from round 1 | ~65 lines | ✅ Done (2026-09-04) | +2–5% GA-active epoch Boltz LE on epoch 2+ (especially Ridge surrogate tier) by seeding crossover with Boltz-validated winners instead of PSICHIC-only candidates |
| 72 | §BBBBBBBBBBBB: Intra-chunk PSICHIC batch surrogate pre-filter — when RF surrogate active (≥100 cache pts), run dual RF surrogate on each 1 000-molecule SAVI-2020 streaming chunk before PSICHIC; keep top-25% (250 mols) by surrogate UCB score; reduces per-chunk PSICHIC calls by 75%, freeing ~15 s CPU/chunk for §MM rounds | ~40 lines | ✅ Done (2026-09-07) | +2–4 §MM rounds/epoch on RF-tier; distinct from §UUUUUUUUUUUU (SALSA) and §RRRRRRRRRRRR (class weights) |
| 73 | §CCCCCCCCCCCC: Residue-type contact map from Boltz PDB for pharmacophore-guided fg_add — extend §ZZZZZZZZZZZZ exposure cache to return (dist, contact_type) tuples where contact_type ∈ {hbd, hba, hpb, other}; bias fg_add FG selection toward pharmacophore-compatible groups at each position (F/O/N near H-bond donor residues; N/O near acceptors; F/Cl/C near hydrophobic pockets) | ~95 lines | ✅ Done (2026-09-07) | +4–8% Boltz LE on epoch 2+ by reducing pharmacophore-mismatched fg_add probes; builds on §ZZZZZZZZZZZZ |
| 74 | §DDDDDDDDDDDD: Fast-mode linear score calibration for §MM acceptance threshold — track (fast_le, full_le) pairs in new `boltz_cache.fast_le` column; when ≥5 pairs available, fit OLS `full_est = a * fast + b`; adjust §MM acceptance gate from `fast > full_best` to `fast > (full_best - b) / a`; corrects systematic bias where fast scores are 0.05–0.20 LE units below full-quality scores | ~45 lines | ✅ Done (2026-09-07) | +2–5% §MM acceptance rate on epoch 2+; largest gain when sampling_steps_affinity gap is wide (A100: 150 full vs 50 fast) |
| 75 | §EEEEEEEEEEEE: §MM diminishing-returns early exit — track last 3 per-round LE improvement deltas; when all 3 < 0.003 LE and a next §YYYYYYYYYYYY seed exists, break inner round loop and advance to next global seed; frees 2–4 rounds/epoch when first seed converges to its local basin | ~22 lines | ⏳ Proposed (2026-09-08) | +2–4% Boltz LE on epoch 2+ warm-cache epochs with ≥3 diverse §YYYYYYYYYYYY seeds; zero regression on cold start or rapid-improvement rounds |
| 76 | §FFFFFFFFFFFFFFFF: GradientBoosting surrogate tier at ≥600 cache pts — add `sklearn.GradientBoostingRegressor` (n_estimators=100, max_depth=4, lr=0.08, subsample=0.8, early-stopping) as 3rd tier above RF; captures non-linear feature interactions (halogen × contact-type × HA-count) that RF's independent trees miss; no new dependencies | ~50 lines | ⏳ Proposed (2026-09-08) | +3–6% surrogate NDCG at 600+ pts (week 2+); cascades to §HHHHHHHHHH pool ranking, §AAAAAA UCB, §BBBBBBBBBBBB chunk pre-filter, §RRRRRRRRRRRR rxn-class weights |
