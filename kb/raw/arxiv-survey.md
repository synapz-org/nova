...
Items 1–69 are **implemented** (§YYYYYYYYYYYY added 2026-09-03). §AAAAAAAAAAAA (item 71) added 2026-09-04: GA population diversity injection from cross-epoch SQLite elite — queries top-10 reliability-adjusted Boltz cache molecules, selects up to 3 by max-min Tanimoto diversity via `_select_diverse_mm_seeds()`, injects into GA seed pool before surrogate augmentation so injected elites participate in crossover from generation 1. Item 5 remains as a conditional direction (binding-pocket docking filter, only relevant when `binding_pocket` is set in config). Item 70 remains proposed: §ZZZZZZZZZZZZ (Boltz-2 binding-pose cache for structure-guided §MM growth vectors, ~150 lines across wrapper.py + salsa.py).
