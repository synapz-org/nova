...
Items 1–63 are **implemented** (§SSSSSSSSSSSS added 2026-08-25: cross-epoch SALSA operator win persistence — operator wins decay ÷2 at epoch boundary instead of resetting to zero; wins are saved to SQLite and GitHub cache so cross-restart and cross-epoch SAR signal is warm from round 1). §RRRRRRRRRRRR added 2026-08-24. §QQQQQQQQQQQQ (Morgan FP 64→256 bits) added 2026-08-23. §OOOO (SALSA bandit operator weighting) added 2026-08-22. §TTTTTTTTTTTT (reaction-class dead-end de-emphasis) added 2026-08-27. Item 5 remains as a conditional direction (binding-pocket docking filter, only relevant when `binding_pocket` is set in config). §UUUUUUUUUUUU (surrogate pre-filter for SALSA perturbations) is proposed but not yet implemented.
